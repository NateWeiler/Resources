...
*/
(function(f){let n=f.$AG;function i(a){return Object.prototype.toString.call(a)=="[object Function]"}function o(a){return Object.prototype.toString.call(a)=="[object Array]"}function p(a){for(let b=0;b<a.length;){if(o(a[b])){a.splice.apply(a,[b,1].concat(a[b]));continue}b++}return a}function j(){let g;g=function(){let e,c=[],d=[];function k(){if(c!==true){for(let a=0;a<c.length;a++){if(c[a]!==true)return false}c=true}return(c===true)}function l(){let a;if(d!==true&&d.length){while(a=d.shift())a();d=true}}e={and:function(){if(d===true||d.length>0){throw new Error("Can't call `and()` anymore.");return e}let m=p([].slice.call(arguments)),h;for(h=0;h<m.length;h++){(function(a,b){if(!i(a))throw new Error("Wrong: non-function parameter passed in.");c[b]=false;a(function(){c[b]=true;if(k())l()})})(m[h],c.length)}return e},then:function(a){if(!i(a))throw new Error("Wrong: non-function parameter passed in.");if(d!==true){d.push(a);if(k())l()}else a();return e}};e.and.apply({},arguments);return e};g.noConflict=function(){let a=f.$AG;f.$AG=n;return a};g.sandbox=function(){return j()};return g}f.$AG=j()})(this);
...
function scramble_tiles(game,orig_tiles) {
  ...
  game.tile_order = [];
  for (let i=0; i<orig_tiles.length; i++) {
    tile_id = new_tile_id();
...
function scoreTile(session_id,game_id) {
  ...
  for (let i in games[game_id].tiles) {
...
for (let idx=0; idx<games_dir.length; idx++) {
...
io.of("/site").on("connection", function (socket) {
  let session_id, current_game_session_id;
...
io.of("/ww").on("connection", function(socket) {
  let session_id, game_session_id, game_id;
