...
(function(b, a) { let c = (navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia); if (c) { b.getUserMedia = function(p, g, k) { if (p !== undefined) { navigator.getUserMedia_ = c; let i = {}; let j = ""; let d, o, f, h, n; if (p.video === true) { i.video = true;
                    j = "video" } if (p.audio === true) { i.audio = true; if (j !== "") { j = j + ", " }
                    j = j + "audio" }
                d = a.getElementById(p.el);
                o = a.createElement("video");
                h = parseInt(d.offsetWidth, 10);
                n = parseInt(d.offsetHeight, 10); if (p.width < h && p.height < n) { p.width = h;
                    p.height = n }
                o.width = p.width;
                o.height = p.height;
                o.autoplay = true;
                ...
                f = o;
                p.videoEl = f;
                p.context = "webrtc"; try { navigator.getUserMedia_(i, g, k) } catch (l) { try { navigator.getUserMedia_(j, g, k) } catch (m) { return undefined } } } } } }(this, document));

// from PHP.js
// https://github.com/kvz/phpjs/
function htmlspecialchars(c, h, g, b) { let e = 0,
        d = 0,
        f = false; if (typeof h === "undefined" || h === null) { h = 2 }
    c = c.toString(); if (b !== false) { c = c.replace(/&/g, "&amp;") }
    c = c.replace(/</g, "&lt;").replace(/>/g, "&gt;"); let a = { ENT_NOQUOTES: 0, ENT_HTML_QUOTE_SINGLE: 1, ENT_HTML_QUOTE_DOUBLE: 2, ENT_COMPAT: 2, ENT_QUOTES: 3, ENT_IGNORE: 4 }; if (h === 0) { f = true } if (typeof h !== "number") { h = [].concat(h); for (d = 0; d < h.length; d++) { if (a[h[d]] === 0) { f = true } else { if (a[h[d]]) { e = e | a[h[d]] } } }
        h = e } if (h & a.ENT_HTML_QUOTE_SINGLE) { c = c.replace(/'/g, "&#039;") } if (!f) { c = c.replace(/"/g, "&quot;") } return c }


(function(global, $) {

    let MAX_FILE_SIZE = 200 * 1024, // 200k max file size
        MIN_IMG_WIDTH = 250,
        MIN_IMG_HEIGHT = 250,
        MAX_IMG_WIDTH = 650,
        MAX_IMG_HEIGHT = 650,
    ...
    function login_needed(page, defer) {
        function checkLoginReq() {
            if (!session_id || !user_info) {
                let current_href = location.href.replace(/^.*?\/([\w0-9\-_]+\.html)/, "$1");
    ...
    function login_complete(data) {
        ...
        let matches = location.href.match(/[#?&]{0,2}from=([^&]+)/),
            redirectHref;
    ...
    function closeCurrentGame(data) {
        let parts = parseUri(location.href);
    ...
    function generateTiles($preview_img, preview_img_type, tile_size) {
        let preview_img = $preview_img.get(0),
    ...
    function buildPreviewGrid($preview_img, width, height, tile_size) {
        ...
        let $preview_grid = $("<canvas />").attr({ id: "preview_grid" }),
    ...
    function processPreview(difficulty, $orig_preview_img) {
        let img_width = $orig_preview_img.width(),
            ...
            delta_width = img_width - MAX_IMG_WIDTH,
            delta_height = img_height - MAX_IMG_HEIGHT,
            img_x = 0,
            img_y = 0,
            img_ratio = 1,
            rows, cols,
    ...
    function readFile(file) {
        ...
        let $preview_container = $("#preview_container");
    ...
    function uploadGame($preview_img) {
        let preview_img_type = $preview_img.attr("data-img-type"),
    ...
    function userJoinGame(data) {
        let name = htmlspecialchars(data.name),
    ...
    function updateGameClock(data) {
        let minutes, seconds, clock_text = "";
        ...
        // clock is ahead/askew... wait/resync
            ...
            let clock_skew = data.time_left - game_clock;
    ...
    function showVideoCaptureControls(evt) {
        let $video_capture = $("#video_capture");
    ...
    function captureSnapshot(evt) {
        let $canvas = $("<canvas></canvas>").attr({ width: 500, height: 410 }),
    ...
    let session_id = retrieveSessionId(),
        ...
        handlers = {
            ...
            "new-puzzle.html": function() {
                ...
                let $file_selector = $("#file_selector"),
    ...
    global.pagePreCheck = function(page) {
        switch (page) {
            ...
            case "new-puzzle.html":
                if (!session_id || !user_info) {
                    let check = login_needed(page, true);
                    if (check !== false) {
